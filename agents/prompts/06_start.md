...
| Choice | Action |
|---|---|
| **1** | Drive Track A in the **visible parent session** (follow `edw-demo-guide` protocol). Remind logins are interactive. **Do not** dump materialize→bootstrap→setup into one mute Cursor `Task`. After catalog: start `track_a_provision.sh` (or `make provision-track-a`) in **that same turn** — do not run a standalone `announce_observability` and stop. Paste `[edw]` / announce blocks as the wrapper prints them. Then coordinator + live observability; Assess/Convert/Test/Gate as `edw-*` subagents. |
