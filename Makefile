...
#
#   Guided demo (human):  az login && databricks auth login → launch edw-demo-guide
#   Scripted:             make demo
#   Existing Azure SQL:   fill .env → make setup && make deploy
#
...
.PHONY: check check-core check-azure render bootstrap setup federation deploy run demo teardown genie materialize-demo sync-prompts discover
...
check-azure: check-core
	...
	@test -n "$(AZ_SUBSCRIPTION_ID)$(AZ_SQL_SERVER)" || \
		{ echo "Azure source vars missing (AZ_SUBSCRIPTION_ID for bootstrap, or AZ_SQL_SERVER for setup)" >&2; exit 1; }
...
federation: render ## UC federation + ops
...
setup: check-core federation deploy genie ## Wire sink + dashboard + genie
	@echo
	@echo "Setup complete. Catalog=$(DATABRICKS_CATALOG)"
