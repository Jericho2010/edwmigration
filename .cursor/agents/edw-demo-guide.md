...
5. **Step migration** — hand off to / drive **`edw-coordinator`** with the live-observability contract:
   ...
   - Remind once: Gate Hero empty until Gate — expected; Inventory/Events/Backlog should move as stages complete; tables-landed (`load_control`) should move at Land. Gate does not end MLflow; coordinator `end-run` at Done. After Land, Workspace **Notebooks** folder `edwmigration_YYYYMMDD` must be in the URL banner (with Catalog + Job).
...
7. **Teardown offer** — Databricks-only (keeps Azure SQL): `make teardown-databricks` (job, dashboard, Genie, MLflow experiment/runs, catalog, foreign catalog, connection, secret scope, `edwmigration_*` notebooks). Azure: `make teardown`. Between demos without destroying assets: `make reset-sink` (chat-confirmed).
