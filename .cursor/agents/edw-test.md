---
name: edw-test
description: Run reconcile.sql against the medallion tables and the legacy fixtures, then return a reconcile_report.json with pass/fail per check. Readonly — returns structured JSON to the coordinator. Launch via the edw-coordinator subagent, not directly.
model: inherit
readonly: true
---
