...
--                 bronze.integration_city_staging, then append staging versions with
--                 lineage_key. SQL Server IDENTITY City Key → max(existing)+row_number
...
new_versions AS (
  SELECT
    (
      SELECT COALESCE(MAX(`City Key`), 0)
