...
CREATE OR REPLACE TABLE __UC_CATALOG__.gold.mart_daily_internet_sales AS
SELECT
...
FROM __UC_CATALOG__.silver.fact_sale f
INNER JOIN __UC_CATALOG__.silver.dim_date d
...
SELECT 'gold_mart_daily_sales_ok' AS check_name,
       (SELECT COUNT(*) FROM __UC_CATALOG__.gold.mart_daily_internet_sales) AS rows;
