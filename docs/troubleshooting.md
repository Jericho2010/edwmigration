...
| `UC_RESOURCE_QUOTA_EXCEEDED` 200 tables / schema limit 100 | Federation as `sa`/`edwadmin` enumerates `sys`. Run `./agents/tools/ensure_federation_login.sh` then `make setup` (creates `edwfed`). |
...
| Dashboard empty / stale | Hooks buffer not flushed, or prior-run ops rows. Flush threshold is now 1 (async from the 15s hook; milestones force-flush). Latest-run widgets prefer `ops.agent_events` then the manifest. If widgets show prior-run reconcile/backlog: `make reset-sink` (Databricks only; keeps Azure). Gate Hero empty until Gate writes `migration_manifest_current`; **Inventory** moves at Land; **tables-landed** (`load_control` `row_count > 0`) moves at **Job**. A Lakeview **Edit** draft with empty X/Y is not the published spec — discard it and `make deploy`. |
...
| Silent agent / empty Dashboard **during** Convert | Assess/Convert/Test/Gate must be **parent-launched** Cursor **`edw-*`** Tasks (not nested under coordinator). This host often omits `subagentStart` — run `record_subagent_hook.sh` in the same turn as the Task. Merge/persist/`observe_status` **exit 1** on Track A without hook `subagentStart`. Dual_write does **not** unblock. |
