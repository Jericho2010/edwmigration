...
| Silent agent / empty Dashboard **during** Convert | Assess/Convert/Test/Gate must be Cursor **`edw-*`** subagents so hooks fire. Merge/persist/`observe_status` **exit 1** on Track A without hook `subagentStart`. Dual_write does **not** unblock. Re-launch `edw-convert`. |
