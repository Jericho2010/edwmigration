...
- **Convert vs job tasks:** Gate verifies notebooks on disk + `ops.proc_conversion_map`. The medallion job (`databricks/jobs/edw_migration_medallion.yml`) runs the **checked-in** silver/gold task paths only — a newly converted notebook is not auto-added as a job task until you extend that YAML. Coordinator/demo-guide run `python3 agents/tools/check_job_wiring.py --run-id <run_id>` (WARN-only) before deploy.
