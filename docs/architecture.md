...
**Subagents:** Assess / Convert / Test / Gate must run as **parent-launched** Cursor **`edw-assess` / `edw-convert` / `edw-test` / `edw-gate`** (not nested under coordinator). When Cursor omits `subagentStart`, the parent runs `record_subagent_hook.sh` in the same turn. Opaque Task / `generalPurpose` fan-out is forbidden. Dual_write is **not** a substitute: `merge_convert_results.py` and persist helpers fail closed without hook `subagentStart`.
